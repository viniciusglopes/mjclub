...
export async function signInWithPhone(formData: FormData) {
  ...
  if (!profile) {
    redirect(
      "/entrar?erro=" +
        encodeURIComponent("Não encontramos esse telefone. Assine um plano do clube para entrar."),
    );
  }
...
export async function signInAsPartner(formData: FormData) {
  ...
  const partner = await getRepository(legacyTenantId()).getPartnerBySlug(slug);
