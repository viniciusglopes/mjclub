    ...
    <div>
          ...
          <ul className="grid gap-3 sm:grid-cols-3">
            {staff
              ...
              .map((p) => (
                <li key={p.id}>
                  <Link
                    href={href(sp, { profissional: p.id })}
                    className="flex h-full flex-col rounded-xl border border-line bg-surface p-5 transition-colors hover:border-gold"
                  >
      ...
      {step > 0 ? (
        <p className="mt-8 text-sm">
          <Link href="/agendar" className="text-muted hover:text-gold">
            ← Recomeçar
          </Link>
        </p>
      ) : null}
...
async function SlotPicker({
  ...
  const slots = await getAvailability({ staffId, serviceId, dateISO });
        ...
        <ul className="grid grid-cols-3 gap-2 sm:grid-cols-6">
          {slots.map((slot) => (
            <li key={slot.startsAt}>
              <Link
                href={href(sp, { horario: slot.startsAt })}
